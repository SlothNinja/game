...
func (h *Header) validateDrop(u *user.User) error {
	...
	case h.Status != Recruiting:
		return fmt.Errorf("game is no longer recruiting, thus %s can't drop: %w", u.Name, sn.ErrValidation)
	case !h.hasUser(u):
		return fmt.Errorf("%s has not joined this game, thus %s can't drop: %w", u.Name, u.Name, sn.ErrValidation)
	default:
		return nil
	}
}
